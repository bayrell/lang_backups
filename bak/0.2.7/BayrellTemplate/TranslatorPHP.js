...
class TranslatorPHP extends BayrellLangTranslatorPHP{
	...
	OpNamespace(op_code){
		...
		if (this.current_module_name != "BayrellRtl"){
			...
			res += this.s("use BayrellWeb\\RenderContainer;");
	...
	OpHtmlTagAttributes(op_code){
		...
		for (var i = 0; i < op_code.attributes.count(); i++){
			...
			if (item.value instanceof OpHtmlExpression){
				...
				s = this.translateRun(item.value);
				if (item.is_json){
					s = "rs::json_encode("+rtl.toString(s)+")";
				}
				else if (item.is_raw){
					s = "rtl::toString("+rtl.toString(s)+")";
				}
				else {
					s = "$container->htmlEscape(rtl::toString("+rtl.toString(s)+"))";
				}
			}
	...
	OpHtmlTag(op_code){
		...
		s += this.s(rtl.toString(this.getCurrentVariable())+" .= $container->out(\"<"+rtl.toString(op_code.tag_name)+"\" . $attrs . \""+rtl.toString((this.isDoubleToken(op_code.tag_name)) ? (">") : ("/>"))+"\");");
		if (op_code.is_plain){
			res += "$container->pushOneLine();";
			res += this.s("$container->setOneLine(true);");
			res += this.s(rtl.toString(this.getCurrentVariable())+" .= $container->outIndent(\"\", true);");
			res += this.s(s);
		}
		else {
			res = s;
		...
		if (this.isDoubleToken(op_code.tag_name)){
			res += this.s("$container->levelInc();");
			if (op_code.childs != null){
				for (var i = 0; i < op_code.childs.count(); i++){
					res += this.s(this.translateRun(op_code.childs.item(i)));
				}
			}
			res += this.s("$container->levelDec();");
			res += this.s(rtl.toString(this.getCurrentVariable())+" .= $container->out(\"</"+rtl.toString(op_code.tag_name)+">\");");
		}
		if (op_code.is_plain){
			res += this.s(rtl.toString(this.getCurrentVariable())+" .= $container->newLine(\"\", true);");
			res += this.s("$container->popOneLine();");
		}
	...
	OpHtmlExpression(op_code){
		...
		for (var i = 0; i < op_code.childs.count(); i++){
			...
			if (item instanceof OpString){
				...
				v.push("$container->htmlEscape(rtl::toString("+rtl.toString(s)+"))");
			}
		}
		return rs.implode(".", v);
	}
	...
	OpHtmlText(op_code){
		...
		s = rtl.toString(this.getCurrentVariable())+" .= $container->out("+rtl.toString(s)+");";
		return s;
	}
	/**
	 * Html Value
	...
	OpHtmlValue(op_code){
		...
		if (!this.is_plain){
			s = "$container->shiftLines("+rtl.toString(s)+")";
		}
	...
	OpHtmlEscape(op_code){
		if (op_code.is_json){
			...
			return "rtl::toString("+rtl.toString(this.translateRun(op_code.value))+")";
		}
		return "$container->htmlEscape(rtl::toString("+rtl.toString(this.translateRun(op_code.value))+"))";
	}
	/**
	 * Html comment
	 */
	OpHtmlComment(op_code){
		...
		return rtl.toString(this.getCurrentVariable())+" .= $container->out("+rtl.toString(this.convertString(s))+");";
	}
	/**
	 * Html View
	 */
	...
	OpRenderRecurseHtmlView(op_code){
		...
		res += this.s("$container->pushLevel();");
		res += this.s("$container->clearLevel();");
		this.variables.push(op_code.variable);
		for (var i = 0; i < op_code.childs.count(); i++){
			var item = op_code.childs.item(i);
			res += this.s(this.translateRun(item));
		}
		res += this.s("$container->popLevel();");
	...
	OpComponent(op_code){
		...
		/* Create components */
		var old_is_operation = this.beginOperation();
		var s0 = "$"+rtl.toString(component_name)+" = new "+rtl.toString(this.translateRun(op_code.name))+"();";
		this.endOperation(old_is_operation);
		/* Output @view */
		...
		s3 += this.s(rtl.toString(this.getCurrentVariable())+" .= "+"$"+rtl.toString(component_name)+"->main($container);");
	...
	OpViewDeclare(op_code){
		...
		res += this.OpClassInit(op_code.view_variables);
		/* Class functions */
		for (var i = 0; i < op_code.childs.count(); i++){
