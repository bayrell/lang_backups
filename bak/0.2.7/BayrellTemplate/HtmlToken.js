...
class HtmlToken extends ParserToken{
	...
	static getSpecialTokens(){
		return (new Vector()).push("@component").push("@declare").push("@render").push("@while").push("@call").push("@for").push("@if").push("@else").push("@elseif").push("@set").push("@json{").push("@raw{").push("@{").push("<!--").push("-->").push("<!").push("</").push("/>");
	}
	/**
	 * Constructor
	 */
