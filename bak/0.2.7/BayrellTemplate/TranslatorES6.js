...
class TranslatorES6 extends BayrellLangTranslatorES6{
	...
	OpHtmlTagAttributes(op_code){
		...
		for (var i = 0; i < op_code.attributes.count(); i++){
			...
			if (item.value instanceof OpHtmlExpression){
				...
				s = this.translateRun(item.value);
				if (item.is_json){
					s = rtl.toString(this.getName("rs"))+".json_encode("+rtl.toString(s)+")";
				}
				else if (item.is_raw){
	...
	OpHtmlEscape(op_code){
		if (op_code.is_json){
			...
			return rtl.toString(this.getName("rtl"))+".toString("+rtl.toString(this.translateRun(op_code.value))+")";
		}
	...
	OpTemplateDeclare(op_code){
		...
		this.endOperation();
		this.levelInc();
		res += this.s("var "+rtl.toString(this.getCurrentVariable())+" = '';");
		/* Default variables */
		for (var i = 0; i < op_code.args.count(); i++){
			var variable = op_code.args.item(i);
