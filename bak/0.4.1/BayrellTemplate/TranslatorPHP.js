...
class TranslatorPHP extends BayrellLangTranslatorPHP{
	...
	OpNamespace(op_code){
		...
		if (this.current_module_name != "Runtime"){
			...
			res += this.s("use WebJSCore\\Lib as WebJSCoreLib;");
			res += this.s("use WebJSCore\\HtmlItems;");
			...
			this.modules.set("WebJSCoreLib", "WebJSCore.Lib");
			this.modules.set("HtmlItems", "WebJSCore.HtmlItems");
		}
		return res;
	}
	/**
	...
	OpHtmlView(op_code){
		this.pushOneLine(false);
		var res = "(new \\WebJSCore\\HtmlItems())";
		op_code.childs.each((item) => {
			res += this.s("->push("+rtl.toString(this.translateRun(item))+")");
		});
		this.popOneLine();
