...
class BayrellParserBay extends BayrellCommonParser {
	 ...
	 */
	matchOpNot(){
		if (this.isLookToken("not")) {
			this.match("not");
			return BayrellCode.op_not(this.matchOpName());
		}
		else if (this.isLookToken("-")) {
			this.match("-");
			...
			this.match("++");
			return BayrellCode.op_pre_inc(this.matchOpName());
		}
		else if (this.isLookToken("--")) {
			this.match("--");
			return BayrellCode.op_pre_dec(this.matchOpName());
		}
	...
	matchMultiply(){
		this.match("*");
		return BayrellCode.op_mult(this.matchOpNot());
	}
	/*
	 * Сканирует деление
	 */
	matchDivide(){
		this.match("/");
		return BayrellCode.op_div(this.matchOpNot());
	}
	/*
	...
	matchFactor(){
		var arr = [];
		rtl.array_push(arr, this.matchOpNot());
	...
	matchLogicExpression(){
		...
		else if (op == "implements") {
			return BayrellCode.op_implements(op, left, right);
		}
		else {
	...
	matchOperator(){
		...
		if (this.isLookToken("return")) {
		...
		else if (this.isLookToken("class") || this.isLookToken("interface")) {
			if (this.isLookToken("interface")) {
			...
			if (this.isLookToken("implements")) {
				...
				var name2 = this.getToken();
				rtl.array_push(implements_arr, name2);
				while (this.isLookToken(",")) {
					this.match(",");
					name2 = this.getToken();
					rtl.array_push(implements_arr, name2);
			...
			if (flags.declare) {
				return BayrellCode.op_nope();
			}
			return BayrellCode.op_declare_class(name, extend_name, implements_arr, code_tree, flags);
		}
		...
		while (!this.eof()) {
			...
			if (this.isLookToken("=")) {
				...
				this.match(")");
				var code_tree = null;
				if (flags["declare"] == false && !this.isLookToken(";")) {
					code_tree = this.matchOperatorsBlock();
					this._mathSemicolon = false;
				}
				else {
