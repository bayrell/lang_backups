...
class TranslatorES6 extends BayrellLangTranslatorES6{
	...
	OpHtmlExpression(op_code){
		...
		for (var i = 0; i < op_code.childs.count(); i++){
			...
			if (item instanceof OpString){
				...
				v.push("this.htmlEscape("+rtl.toString(this.getName("rtl"))+".toString("+rtl.toString(s)+"))");
			}
		}
		return rs.implode("+", v);
	}
	...
	OpComponent(op_code){
		...
		/* Create components */
		var old_is_operation = this.beginOperation();
		var s0 = "var "+rtl.toString(component_name)+" = new "+rtl.toString(this.translateRun(op_code.name))+"(this.context(), this.renderContainer());";
		this.endOperation(old_is_operation);
		/* Output @view */
