...
Object.assign(Runtime.MessageRPC,
{
	...
	{
		var __v0 = use("Runtime.RuntimeConstant");
		return msg.copy(ctx, use("Runtime.Dict").from({"code":__v0.ERROR_OK,"error":"","response":response}));
	},
	/**
	 * Set fail result
	 * @param primitive res
	...
	fail: function(ctx, error, code, response, msg)
	{
		...
		return msg.copy(ctx, use("Runtime.Dict").from({"code":code,"error":error,"response":response}));
	},
