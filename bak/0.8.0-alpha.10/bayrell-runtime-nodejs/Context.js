...
Object.assign(Runtime.Context,
{
	...
	getDriversFromEntities: function(context)
	{
		...
		for (var i = 0;i < arr.count(context);i++)
		{
			...
			driver = this.chain(class_name, use("Runtime.Collection").from([driver]), context);
			if (class_name != driver_name)
			{
				driver = this.chain(driver_name, use("Runtime.Collection").from([driver]), context);
			}
	 ...
	 */
	getDriver: function(driver_name, context)
	{
	 ...
	 * @return CoreProvider
	 */
	createProvider: function(provider_name, params, settings_name, context)
	{
		var provider = null;
		if (context.providers.has(context, provider_name))
		{
			...
			{
				var __v0 = use("Runtime.rtl");
				params = __v0.attr(context, context.settings, use("Runtime.Collection").from(["providers",class_name,settings_name]));
			}
			if (params == null)
			{
			...
			provider = this.chain(class_name, use("Runtime.Collection").from([provider]), context);
			if (provider_name != class_name)
			{
				provider = this.chain(provider_name, use("Runtime.Collection").from([provider]), context);
			}
		}
		else
		{
	...
	getProvider: function(provider_name, settings_name, context)
	{
		...
		var provider = this.createProvider(provider_name, null, settings_name, context);
	...
	sendMessage: function(msg, context)
	{
		...
		return (__async_t) =>
		{
			if (__async_t.pos() == "0")
			{
				...
				provider = this.getProvider(__v0.BUS_INTERFACE, "default", context);
				return __async_t.jump(__async_t,"1").call(__async_t,provider.constructor.sendMessage(context, msg, provider),"__v0");
			}
			else if (__async_t.pos() == "1")
			{
				msg = __async_t.getVar("__v0");
