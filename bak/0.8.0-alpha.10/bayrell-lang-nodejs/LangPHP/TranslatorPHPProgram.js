	...
	OpDeclareClassBody: function(ctx, t, op_code)
	{
		...
		if (op_code.functions != null)
		{
			for (var i = 0;i < op_code.functions.count(ctx);i++)
			{
				...
				s1 = t.s(ctx, s1);
				/* Function comments */
				var res = t.operator.constructor.AddComments(ctx, t, f.comments, s1 + use("Runtime.rtl").toStr(s2));
				content += use("Runtime.rtl").toStr(res[1]);
			}
		}
		var __v0 = use("Bayrell.Lang.OpCodes.OpDeclareClass");
		...
		for (var i = 0;i < op_code.items.count(ctx);i++)
		{
			...
			{
				for (var j = 0;i < item.items.count(ctx);i++)
				{
					var res = t.operator.constructor.OpPreprocessorIfCode(ctx, t, item.items.item(ctx, i));
					var s = res[1];
					if (s == "")
					{
						continue;
