...
class BayrellParserBay extends BayrellSimpleParser {
	
	constructor(){
		super();
		this.mathSemicolon = true;
		this.pair_tokens = ["!==", "===", "!=", "==", "<=", ">=", "::", "&&", "||", ">>", "<<", "->", "++", "--", "+=", "-=", "*=", "/=", "#ifcode", "#switch", "#case", "#endswitch"];
		this.string_tokens = ["\"", "'"];
	...
	*/
	...
	matchName(){
		...
		if (this.look_token_type == "string") {
			...
			name = this.getToken();
			throw this.createError(BayrellParserBay.ERROR_IDENT_IS_RESERVER_WORD, {'name': name,
			});
		}
			...
			*/
			...
			if (this.isLookToken("(")) {
				...
				while (this.isLookToken("[")) {
					this.match("[");
					array_push(pos, this.matchArithmetic());
					this.match("]");
				}
			...
			var values = [];
			while (!this.isLookToken("]") && !this.eof()) {
				array_push(values, this.matchArithmetic());
				if (this.isLookToken(",")) {
					this.match(",");
				}
			}
			...
			name = this.getToken();
			throw this.createError(BayrellParserBay.ERROR_UNKNOWN_IDENT, {'name': name,
			});
		}
		return result;
	}
	...
	*/
	...
	matchMultiply(){
		this.match("*");
		return BayrellCode.op_mult(this.matchNamePost());
	}
	/*
	Сканирует деление
	*/
	...
	matchDivide(){
		this.match("/");
		return BayrellCode.op_div(this.matchNamePost());
	}
	/*
	...
	*/
	...
	matchFactor(){
		var arr = [];
		array_push(arr, this.matchNamePost());
		while (this.isLookTokenArr(["*", "/"]) && !this.eof()) {
			if (this.isLookToken("*")) {
				...
				throw this.createError(BayrellParserBay.ERROR_EXPECTED, {'what': '"*" or "/"',
				});
			}
		}
		if (count(arr) == 1) {
	...
	*/
	...
	matchPreprocessorIfCode(){
		...
		if (this.isLookToken("then")) {
			throw this._createError(BayrellCommonParser.ERROR_EXPECTED, {'what': 'then',
			});
		}
	...
	*/
	...
	matchPreprocessorDirective(){
		...
		throw this.createError(BayrellParserBay.ERROR_UNKNOWN_PREPROCESSOR_DIRECTIVE, {'name': this.look_token,
		});
		return null;
	}
	/*
		...
		if (first_name == "return") {
			...
			*/
			...
			if (this.isLookTokenIdentifier()) {
				...
				throw this.createError(BayrellParserBay.ERROR_UNKNOWN_IDENT, {'name': this.look_token,
				});
			}
			this.match(";");
