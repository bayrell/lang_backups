...
Object.assign(Bayrell.Lang.LangBay.ParserBayPreprocessor,
{
	...
	readPreprocessorSwitch: function(parser)
	{
		...
		while (token.content == "#case")
		{
			...
			var ifcode = new _v0(use("Runtime.Dict").create({"condition":condition,"content":content,"caret_start":caret_content,"caret_end":parser.caret}));
			items.push(ifcode);
		}
		/* Restore vars */
		...
		parser = res[0];
		var _v0 = use("Bayrell.Lang.OpCodes.OpPreprocessorSwitch");
		return use("Runtime.Collection").create([parser,new _v0(use("Runtime.Dict").create({"items":items.toCollection(),"caret_start":caret_start,"caret_end":parser.caret}))]);
	},
