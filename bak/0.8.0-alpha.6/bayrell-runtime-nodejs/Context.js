...
Object.assign(Runtime.Context,
{
	...
	getModulesEntities: function(modules)
	{
		...
		for (var i = 0;i < modules.count();i++)
		{
			...
			var _v0 = use("Runtime.rtl");
			var f = _v0.method(module_class_name, "entities");
			var arr = f();
		...
		for (var i = 0;i < entities.count();i++)
		{
			...
			if (item1_class_name == "Runtime.Annotations.Entity")
			{
				...
				for (var j = 0;j < info.class_info.count();j++)
				{
					...
					var _v0 = use("Runtime.Annotations.Entity");
					if (item2 instanceof _v0 && item2_class_name != "Runtime.Annotations.Entity")
					{
	...
	start: function(context)
	{
		...
		return (__async_t) =>
		{
			...
			else if (__async_t.pos() == "1.2")
			{
				...
				return __async_t.jump(__async_t,"1.3").call(__async_t,driver.startDriver(),"_v0");
			}
	...
	getDriversFromEntities: function(context)
	{
		...
		for (var i = 0;i < arr.count();i++)
		{
			...
			var _v0 = use("Runtime.rtl");
			var driver = _v0.newInstance(class_name, use("Runtime.Collection").create([context]));
	...
	createProvider: function(context,provider_name,params)
	{
		...
		if (context.providers.has(provider_name))
		{
			...
			var _v0 = use("Runtime.rtl");
			provider = _v0.newInstance(class_name, use("Runtime.Collection").create([params]));
	...
	chain: function(context,chain_name,args)
	{
		...
		for (var i = 0;i < entities.count();i++)
		{
			...
			else
			{
				...
				var _v0 = use("Runtime.rtl");
				var f = _v0.method(class_name, method_name);
				var _v0 = use("Runtime.rtl");
				var res = _v0.apply(f, args);
				args = args.setIm(args.count() - 1, res);
			}
		}
		var res = args.last();
	...
	chainAwait: function(context,chain_name,args)
	{
		...
		return (__async_t) =>
		{
			...
			{
				return __async_t.jump(__async_t,"1.3.3").call(__async_t,this.chainAwait(context, item_chain_name, args),"_v0");
			}
			else if (__async_t.pos() == "1.3.3")
			{
				res = __async_t.getVar("_v0");
				args = args.setIm(args.count() - 1, res);
			...
			else if (__async_t.pos() == "1.3.2")
			{
				/* If false */
				var _v0 = use("Runtime.rs");
				arr = _v0.split("::", item.value);
				class_name = arr.get(0, "");
				method_name = arr.get(1, "");
				var _v0 = use("Runtime.rtl");
				f = _v0.method(class_name, method_name);
			...
			{
				var _v0 = use("Runtime.rtl");
				return __async_t.jump(__async_t,"1.3.4.3").call(__async_t,_v0.apply(f, args),"_v1");
			}
			else if (__async_t.pos() == "1.3.4.3")
			{
				res = __async_t.getVar("_v1");
				args = args.setIm(args.count() - 1, res);
