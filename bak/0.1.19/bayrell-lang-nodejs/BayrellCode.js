...
class BayrellCode{
    ...
    /*
	 * Пробегается по всему коду и возвращает имена в op_load_names_calc
	 * @param {json} code_tree - Дерево кода
	 * @return {json} имена переменных
	 */
    static getValuesFromCodeTree(code_tree){
        var res = {
            "names": [],
        };
        if (code_tree["op"] == BayrellCode.OP_LOAD) {
            rtl.array_push(res["names"], code_tree["str_name"]);
        }
        else if (rtl.in_array(
            code_tree["op"],
            [BayrellCode.OP_LOAD_NAMES, BayrellCode.OP_LOAD_NAMES_CALC]
            ...
            for (var i = 0; i < sz; i++) {
                ...
                if (code["op"] == BayrellCode.OP_LOAD) {
                    ...
                    if (code["pos"]["op"] == BayrellCode.OP_FIXED) {
                        rtl.array_push(_res, "[" + rtl.toString(code["pos"]["str"]) + "]");
                    }
                    else {
                        no_add = true;
                    }
                    var r = BayrellCode.getValuesFromCodeTree(code["pos"]);
                    res["names"] = rtl.array_merge(res["names"], r["names"]);
                }
                else if (code["op"] == BayrellCode.OP_LOAD_STATIC) {
                    rtl.array_push(_res, "." + rtl.toString(rtl.toString(str_name)));
                }
                    ...
                    no_add = true;
                    var r = BayrellCode.getValuesFromCodeTree(code);
                    res["names"] = rtl.array_merge(res["names"], r["names"]);
                }
            }
            if (!no_add) {
                rtl.array_push(res["names"], rtl.implode("", _res));
            }
        }
        else if (rtl.key_exists(code_tree, "childs")) {
            var childs = code_tree["childs"];
            var sz = rtl.count(childs);
            for (var i = 0; i < sz; i++) {
                var r = BayrellCode.getValuesFromCodeTree(childs[i]);
                res["names"] = rtl.array_merge(res["names"], r["names"]);
            }
        }
        else if (rtl.key_exists(code_tree, "value")) {
            var r = BayrellCode.getValuesFromCodeTree(code_tree["value"]);
            res["names"] = rtl.array_merge(res["names"], r["names"]);
        }
        return res;
    }
