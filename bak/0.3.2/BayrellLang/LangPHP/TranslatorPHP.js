...
class TranslatorPHP extends CommonTranslator{
	...
	OpMethod(op_code){
		if (op_code.value instanceof OpDynamic){
			...
			return "new \\BayrellRtl\\Types\\Callback("+rtl.toString(obj)+", "+rtl.toString(this.convertString(name))+")";
		}
		else if (op_code.value instanceof OpStatic){
			var name = op_code.value.name;
			var obj = this.translateRun(op_code.value.value);
			return "new \\BayrellRtl\\Types\\Callback("+rtl.toString(obj)+"::class, "+rtl.toString(this.convertString(name))+")";
		}
