...
class BayrellTranslator extends BayrellObject {
	...
	out(s, level){
		return rtl.str_repeat(this._indent, level) + rtl.toString(s) + rtl.toString(this._clr);
	}
	...
	run(code_tree, level){
		...
		else if (is_json && op == BayrellCode.OP_DEL) {
		...
		else if (is_json && op == BayrellCode.OP_IMPLEMENTS) {
			_res = this.op_implements(code_tree, level);
		}
