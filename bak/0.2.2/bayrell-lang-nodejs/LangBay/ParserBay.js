		...
		if (this.findNextTokenVector(flags_vector) != -1){
			...
			while (this.findNextTokenVector(flags_vector) != -1){
				if (!flags.assignFlag(this.lookNextToken())){
					this.parserError("Unknown flag '"+rtl.toString(this.lookNextToken())+"'");
				}
				this.readNextToken();
			}
		}
	...
	readDeclareClassFunction(){
		...
		while (!this.findNextToken(")") && !this.isEOF()){
			...
			if (op_code instanceof OpAssign){
				this.parserError("Assign are not alowed here");
			}
	...
	readClassBody(res){
		...
		while (!this.findNextToken("}") && !this.isEOF()){
			if (this.findNextTokenPreprocessor()){
				res.class_functions.push(this.readPreprocessor());
				continue;
			}
			else if (this.lookNextTokenType() == ParserBayToken.TOKEN_COMMENT){
				res.class_functions.push(new OpComment(this.readAnyNextToken().token));
				continue;
			}
			...
			if (op_code){
				op_code.flags = flags;
				res.class_functions.push(op_code);
				continue;
			}
			...
			this.parserError("Unknown operator");
		}
		this.matchNextToken("}");
	}
	/**
	...
	readPreprocessor(){
		if (this.findNextToken("#switch")){
			...
			while (this.findNextToken("#case")){
				...
				if (pos == -1){
					this.parserError("Unknown preprocessor token "+rtl.toString(this.lookNextToken()));
				}
			}
	...
	readProgram(){
		...
		while (!this.isEOF()){
			...
			if (this.findNextToken("class")){
				...
				this.parserError("Unknown token "+rtl.toString(this.lookNextToken()));
			}
		}
		return res;
	}
