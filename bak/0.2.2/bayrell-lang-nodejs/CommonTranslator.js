	...
	translateRun(op_code){
		if (op_code instanceof OpNope){
			...
			for (var i = 0; i < op_code.childs.count(); i++){
				this.current_opcode_level = 0;
				s = this.translateRun(op_code.childs[i]);
				if (s == ""){
					continue;
				}
				if (flag){
					res += this.translateRun(op_code.childs[i]);
					flag = false;
				}
				else {
					res += this.s(this.translateRun(op_code.childs[i]));
				}
			}
			return res;
		}
		...
		else if (op_code instanceof OpArray){
			return this.OpArray(op_code);
		}
