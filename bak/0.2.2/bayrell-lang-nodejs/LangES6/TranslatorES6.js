...
class TranslatorES6 extends CommonTranslator{
	...
	OpCall(op_code){
		...
		if (op_code.value instanceof OpIdentifier){
			if (op_code.value.value == "parent" && this.current_function_name != "constructor"){
				s += "super."+rtl.toString(this.current_function_name);
				f = false;
			}
		}
		if (f){
	...
	OpClassDeclare(op_code){
		...
		/* Init function */
		if (this.current_namespace != "BayrellRtl" || op_code.class_name != "CoreObject"){
			res += this.s("_init(){");
			this.levelInc();
			if (op_code.class_extends != ""){
		...
		for (var i = 0; i < op_code.class_functions.count(); i++){
			var op_code2 = op_code.class_functions.item(i);
