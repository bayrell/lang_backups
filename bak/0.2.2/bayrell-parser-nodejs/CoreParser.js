...
class CoreParser extends ContextObject{
	...
	readNextToken(tp, error_message){
		if (tp == undefined) tp="token";
		if (error_message == undefined) error_message="Next token";
		if (this.lookNextTokenType() != tp){
			this.nextTokenExpected(error_message);
		}
	...
	nextTokenExpected(message){
		...
		if (message == "\n"){
			throw new ParserExpected(this.context(), "new line", start_line, start_col);
		}
		else {
			throw new ParserExpected(this.context(), message, start_line, start_col);
		}
	}
	/**
	 * Throws expected error
	...
	parserError(message){
		var start_line = this.next_token.start_line;
		var start_col = this.next_token.start_col;
		throw new ParserError(this.context(), message, start_line, start_col);
	}
	/**
	 * Return next token
	...
	matchNextToken(look_str, tp){
		...
		this.nextTokenExpected(look_str);
	}
	/**
	 * Tokens Fabric
