...
class rtl{
	 ...
	 * @return {string} 
	 */
	/*  declare export static var call_user_func_array(var callback, array params); */
	/*  declare export static var call_user_func(var callback); */
	/*  ===================== Работа со строками ===================== */
	/* 
	 ...
	 */
	static substr(s, start, len = null){
		s = rtl.toString(s);
	 ...
	 * @param {string} s - строка 
	 * @param {json} res - результат 
	 * @return {boolean} Истина, если поиск успешен
	 */
	static preg_match(regex, s, res){
	 ...
	 */
	static preg_replace(pattern, replace, subject, limit){
		if (!rtl.isset(limit)) limit = -1;
	...
	static format(s, params){
		...
		for (var key in params){
			...
			array_push(keys, "%" + rtl.toString(key) + "%");
	...
	static microtime(get_as_float){
		if (!rtl.isset(get_as_float)) get_as_float = false;
	 ...
	 * @return {int} результат
	 */
	static gmtimedb(format, timestamp){
		if (!rtl.isset(timestamp)) timestamp = time();
		return "";
	}
	/* 
	...
	static gmtimedb(timestamp){
		if (!rtl.isset(timestamp)){timestamp = -1;}
		if (timestamp < 0) {
			timestamp = time();
		}
 ...
 */
	 ...
	 * @return {boolean} Истина, если ключ есть
	 */
	static key_zero(arr, key){
		if (typeof arr == 'object' || Array.isArray(arr)){
			return rtl.iszero(arr[key]);
		}
		return true;
	}
	/* 
	 ...
	 * @return {boolean} Истина, если ключ есть
	 */
	static key_nozero(arr, key){
		if (typeof arr == 'object' || Array.isArray(arr)){
			return rtl.nozero(arr[key]);
		}
		return false;
	}
	/* 
	...
	static mb_pathinfo(filepath) {
		var ret = [], m = [];
		rtl.preg_match(new RegExp('^(.*?)[\\\\/]*(([^/\\\\]*?)(\.([^\.\\\\/]+?)|))[\\\\/\.]*$', 'im'), filepath, m);
