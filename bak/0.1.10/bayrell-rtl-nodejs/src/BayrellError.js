...
class BayrellError extends Error {
	...
	static createError(name, params, file_name, line, pos){
		if (!rtl.isset(params)){params = {};}
		if (!rtl.isset(file_name)){file_name = null;}
		if (!rtl.isset(line)){line = null;}
		if (!rtl.isset(pos)){pos = null;}
		var s;
		var code = -1;
		s = BayrellTranslate.translate(name, params);
		if (s == "" && rtl.isset(name[1])) {
			s = name[1];
		}
		if (rtl.is_array(name) && rtl.isset(name[2])) {
			code = name[2];
		}
