...
class BayrellCode{
	...
	static out(s, level, crlf, ident){
		if (!rtl.isset(crlf)){crlf = true;}
		if (!rtl.isset(ident)){ident = true;}
		var res = "";
		if (ident) {
	...
	static compare(code1, code2){
		...
		if (rtl.is_array(code1) && rtl.is_array(code2)) {
			...
			while (i < sz) {
				...
				else if ((rtl.key_exists(code1, name) && !rtl.key_exists(code2, name)) || (!rtl.key_exists(code1, name) && isset(rtl.key_exists, name))) {
					return false;
				}
			}
			/*  Массивы */
