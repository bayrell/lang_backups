...
class BayrellTranslator extends BayrellError {
	...
	run(code_tree, level){
		...
		else if (code_tree["op"] == BayrellCode.OP_LOAD_NAMES) {
			result = this.op_load_names(code_tree, level);
		}
		else if (code_tree["op"] == BayrellCode.OP_LOAD_NAMES_CALC) {
			result = this.op_load_names_calc(code_tree, level);
		}
		...
		else if (code_tree["op"] == BayrellCode.OP_ASSIGN_CONCAT) {
			result = this.op_assign_concat(code_tree, level);
		}
