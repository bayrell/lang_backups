...
class BayrellCommonParser extends BayrellObject {
	...
	_createError(error, params, line, col){
		...
		return BayrellError.createError(error, params, this.file_name, line, col);
	}
	_match(look_str){
	...
	_moveCursorPos(ch, invert){
		...
		if (ch == "\n") {
			...
			this._content_cursor_pos = {
				...
				"col":this._content_cursor_pos["col"] + this.tab_space_count * invert,
			};
		}
		else {
			this._content_cursor_pos = {
	...
	isLookToken(token, type){
		if (!isset(type)){type = "token";}
		return this.look_token == token && type == this.look_token_type;
	}
	isLookComment(){
		return this.look_token_type == BayrellCommonParser.TOKEN_COMMENT;
	}
