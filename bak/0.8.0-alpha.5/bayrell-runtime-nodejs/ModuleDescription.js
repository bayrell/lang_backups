...
Object.assign(Runtime.ModuleDescription,
{
	...
	entities: function()
	{
		...
		var _v1 = use("Runtime.Annotations.Provider");
		var _v2 = use("Runtime.Annotations.LambdaChainDeclare");
		return use("Runtime.Collection").create([new _v0(use("Runtime.Dict").create({"name":"Runtime.Interfaces.LocalBusInterface","kind":"interface"})),new _v1(use("Runtime.Dict").create({"name":"Runtime.Interfaces.RemoteBusInterface","kind":"interface"})),new _v2(use("Runtime.Dict").create({"name":"Runtime.Entities"}))]);
	},
	/**
	 * Returns enities
	 */
