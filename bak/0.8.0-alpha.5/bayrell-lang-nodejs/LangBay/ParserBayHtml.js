...
Bayrell.Lang.LangBay.ParserBayHtml.prototype = Object.create(use("Bayrell.Lang.CoreParser").prototype);
...
Object.assign(Bayrell.Lang.LangBay.ParserBayHtml.prototype,
{
	...
	{
		use("Bayrell.Lang.CoreParser").prototype.assignValue.call(this,k,v);
	},
...
Object.assign(Bayrell.Lang.LangBay.ParserBayHtml, use("Bayrell.Lang.CoreParser"));
Object.assign(Bayrell.Lang.LangBay.ParserBayHtml,
{
	...
	readCssSelector: function(parser)
	{
		...
		var ch = _v0.substr(content.ref, pos, 1);
		if (ch == "(")
		{
			pos = pos + 1;
			...
			while (pos < content_sz && ch != ")")
			{
				...
				y = parser.parser_base.constructor.nextY(parser, ch, y);
				var _v0 = use("Runtime.rs");
				ch = _v0.substr(content.ref, pos, 1);
			}
			var _v0 = use("Runtime.rs");
			class_name = _v0.substr(content.ref, start_pos, pos - start_pos);
		...
		var _v0 = use("Runtime.rs");
		var postfix = _v0.substr(content.ref, start_pos, pos - start_pos);
		var _v0 = use("Runtime.RuntimeUtils");
		var selector = "." + use("Runtime.rtl").toString(postfix) + use("Runtime.rtl").toString("-") + use("Runtime.rtl").toString(_v0.getCssHash(class_name));
		var _v0 = use("Bayrell.Lang.Caret");
		var caret = new _v0(use("Runtime.Dict").create({"x":x,"y":y,"pos":pos}));
	...
	readCss: function(parser)
	{
		...
		while (pos < content_sz && (ch != "}" || ch == "}" && bracket_level > 0))
		{
			...
			if (ch == "%")
			{
				...
				/* Add value */
				var _v0 = use("Runtime.rs");
				var value = _v0.substr(content.ref, start_pos, pos - start_pos - 1);
				if (value != "")
				{
			...
			else if (ch == "{")
			{
				/* Add value */
				var _v0 = use("Runtime.rs");
				var value = _v0.substr(content.ref, start_pos, pos - start_pos);
				if (value != "")
				{
					css_str += use("Runtime.rtl").toString(value);
				}
		...
		parser = res[0];
		var _v0 = use("Runtime.rs");
		css_str = _v0.replace("\t", "", css_str);
		var _v0 = use("Runtime.rs");
		css_str = _v0.replace("\n", "", css_str);
		var _v0 = use("Bayrell.Lang.OpCodes.OpString");
		var op_code = new _v0(use("Runtime.Dict").create({"caret_start":caret,"caret_end":parser.caret,"value":css_str}));
	...
	readHTMLValue: function(parser)
	{
		...
		else if (ch == "@")
		{
			...
			pos = pos + 1;
			var _v0 = use("Runtime.rs");
			var ch3 = _v0.substr(content.ref, pos, 3);
			var _v0 = use("Runtime.rs");
			var ch4 = _v0.substr(content.ref, pos, 4);
			if (ch3 == "raw" || ch4 == "json")
			{
				var res;
				if (ch3 == "raw")
			...
			var _v0 = use("Bayrell.Lang.Caret");
			caret = new _v0(use("Runtime.Dict").create({"x":x,"y":y,"pos":pos}));
			parser = parser.copy({ "caret": caret });
			var res = parser.parser_base.constructor.matchToken(parser, "{");
			parser = res[0];
			var res = parser.parser_expression.constructor.readExpression(parser);
			...
			if (ch3 == "raw")
			{
				...
				item = new _v0(use("Runtime.Dict").create({"kind":_v1.KIND_RAW,"value":item,"caret_start":caret,"caret_end":parser.caret}));
			}
			else if (ch4 == "json")
			{
				var _v0 = use("Bayrell.Lang.OpCodes.OpHtmlValue");
				var _v1 = use("Bayrell.Lang.OpCodes.OpHtmlValue");
				item = new _v0(use("Runtime.Dict").create({"kind":_v1.KIND_JSON,"value":item,"caret_start":caret,"caret_end":parser.caret}));
			}
			var res = parser.parser_base.constructor.matchToken(parser, "}");
			parser = res[0];
		}
	...
	readHTMLAttrs: function(parser)
	{
		...
		var content = parser.content;
		var content_sz = parser.content_sz;
		var caret = parser.parser_base.constructor.skipChar(parser, content.ref, parser.caret);
		var _v0 = use("Runtime.rs");
		var ch = _v0.substr(content.ref, caret.pos, 1);
		while (ch != ">" && caret.pos < content_sz)
		{
			...
			parser = res[0];
			var value = res[1];
			var _v0 = use("Bayrell.Lang.OpCodes.OpHtmlAttribute");
			items.push(new _v0(use("Runtime.Dict").create({"key":key,"value":value,"caret_start":caret_start,"caret_end":parser.caret})));
			caret = parser.parser_base.constructor.skipChar(parser, content.ref, parser.caret);
			var _v0 = use("Runtime.rs");
			ch = _v0.substr(content.ref, caret.pos, 1);
			var _v0 = use("Runtime.rs");
			var ch2 = _v0.substr(content.ref, caret.pos, 2);
			if (ch2 == "/>")
			{
				break;
			}
	...
	readHTMLContent: function(parser,end_tag)
	{
		...
		var start_pos = pos;
		var _v0 = use("Runtime.rs");
		var end_tag_sz = _v0.strlen(end_tag);
		var _v0 = use("Runtime.rs");
		var ch2 = _v0.substr(content.ref, pos, end_tag_sz);
		while (ch2 != end_tag && pos < content_sz)
		{
			...
			if (ch == "<" || ch == "{" || ch == "@")
			{
				...
				caret = new _v0(use("Runtime.Dict").create({"x":x,"y":y,"pos":pos}));
				var _v0 = use("Runtime.rs");
				value = _v0.trim(value, "\t\r\n");
				if (value != "")
				{
					var _v0 = use("Bayrell.Lang.OpCodes.OpHtmlContent");
					item = new _v0(use("Runtime.Dict").create({"value":value,"caret_start":caret_start,"caret_end":caret}));
					items.push(item);
				}
				/* Read HTML Value */
				parser = parser.copy({ "caret": caret });
			...
			var _v0 = use("Runtime.rs");
			ch2 = _v0.substr(content.ref, pos, end_tag_sz);
		}
		/* Push item */
		var _v0 = use("Runtime.rs");
		var value = _v0.substr(content.ref, start_pos, pos - start_pos);
	...
	readHTMLTag: function(parser)
	{
		...
		if (token.content == "{")
		{
			...
			var _v0 = use("Runtime.rs");
			tag_name = _v0.substr(parser.content.ref, caret1.pos, caret2.pos - caret1.pos);
		}
		else
		{
		...
		var _v0 = use("Bayrell.Lang.OpCodes.OpHtmlTag");
		var _v1 = use("Bayrell.Lang.OpCodes.OpHtmlItems");
		var op_code = new _v0(use("Runtime.Dict").create({"attrs":attrs,"tag_name":tag_name,"op_code_name":op_code_name,"caret_start":caret_start,"caret_end":parser.caret,"items":(items != null) ? new _v1(use("Runtime.Dict").create({"caret_start":caret_items_start,"caret_end":caret_items_end,"items":items.toCollection()})) : null}));
		return use("Runtime.Collection").create([parser,op_code]);
	},
	/**
	 * Read html template
	...
	readHTML: function(parser)
	{
		...
		var _v0 = use("Runtime.Vector");
		var items = new _v0();
		var caret_start = parser.caret_start;
		var res = parser.parser_base.constructor.readToken(parser);
		look = res[0];
		token = res[1];
		var _v0 = use("Runtime.rs");
		var ch2 = _v0.substr(parser.content.ref, parser.caret.pos, 2);
		while (!token.eof && token.content == "<" && ch2 != "</")
		{
			...
			var _v0 = use("Runtime.rs");
			ch2 = _v0.substr(parser.content.ref, caret.pos, 2);
		}
		var _v0 = use("Bayrell.Lang.OpCodes.OpHtmlItems");
		var op_code = new _v0(use("Runtime.Dict").create({"caret_start":caret_start,"caret_end":parser.caret,"items":items.toCollection()}));
		return use("Runtime.Collection").create([parser,op_code]);
	},
