...
Object.assign(Bayrell.Lang.LangBay.ParserBayOperator,
{
	...
	readTry: function(parser)
	{
		...
		while (!token.eof && token.content == "catch")
		{
			...
			var _v0 = use("Bayrell.Lang.OpCodes.OpTryCatchItem");
			var item = new _v0(use("Runtime.Dict").create({"name":var_name,"pattern":pattern,"value":op_catch,"caret_start":item_caret_start,"caret_end":parser.caret}));
			items.push(item);
			var res = parser.parser_base.constructor.readToken(parser);
			look = res[0];
			token = res[1];
		}
		var _v0 = use("Bayrell.Lang.OpCodes.OpTryCatch");
		return use("Runtime.Collection").create([parser,new _v0(use("Runtime.Dict").create({"op_try":op_try,"items":items.toCollection(),"caret_start":caret_start,"caret_end":parser.caret}))]);
	},
	/**
	 * Read then
	 */
	...
	readIf: function(parser)
	{
		...
		while (!token.eof && (token.content == "else" || token.content == "elseif"))
		{
			...
			if (token2.content == "elseif" || token2.content == "if")
			{
				...
				parser = res[0];
				ifelse_block = res[1];
				var _v0 = use("Bayrell.Lang.OpCodes.OpIfElse");
				if_else.push(new _v0(use("Runtime.Dict").create({"condition":ifelse_condition,"if_true":ifelse_block,"caret_start":token2.caret_start,"caret_end":parser.caret})));
			}
			else
			{
		...
		var _v0 = use("Bayrell.Lang.OpCodes.OpIf");
		return use("Runtime.Collection").create([parser,new _v0(use("Runtime.Dict").create({"condition":if_condition,"if_true":if_true,"if_false":if_false,"if_else":if_else.toCollection(),"caret_start":caret_start,"caret_end":parser.caret}))]);
	},
	/**
	 * Read For
	 */
	...
	readAssign: function(parser)
	{
		...
		if (token.content == "<=")
		{
			...
			while (!token.eof && token.content == "<=")
			{
				...
				else
				{
					if (!parser.parser_base.constructor.isIdentifier(name))
					{
						...
						throw new _v0("Identifier",save.caret,parser.file_name)
					}
					arr.push(name);
				}
			}
			...
			parser = res[0];
			expression = res[1];
			var _v0 = use("Bayrell.Lang.OpCodes.OpAssignStruct");
			return use("Runtime.Collection").create([parser,new _v0(use("Runtime.Dict").create({"caret_start":caret_start,"caret_end":parser.caret,"expression":expression,"var_name":var_name,"names":names}))]);
		}
		if (token.content != "=" && token.content != "+=" && token.content != "-=" && token.content != "~=" && token.content != "." && token.content != "::" && token.content != "[")
		{
			...
			parser = parser.copy({ "vars": parser.vars.setIm(var_name, true) });
			var _v0 = use("Bayrell.Lang.OpCodes.OpAssignValue");
			values.push(new _v0(use("Runtime.Dict").create({"var_name":var_name,"expression":expression,"caret_start":var_op_code.caret_start,"caret_end":parser.caret})));
			/* Look next token */
			var res = parser.parser_base.constructor.readToken(parser);
			look = res[0];
			token = res[1];
		...
		else
		{
			...
			else
			{
				...
				throw new _v0("Unknown operator " + use("Runtime.rtl").toString(token.content),token.caret_start,parser.file_name)
			}
			var res = parser.parser_expression.constructor.readExpression(parser);
			parser = res[0];
			expression = res[1];
			var _v0 = use("Bayrell.Lang.OpCodes.OpAssignValue");
			values = use("Runtime.Collection").create([new _v0(use("Runtime.Dict").create({"op_code":op_code,"expression":expression,"op":op}))]);
			var_name = "";
			expression = null;
		}
		var _v0 = use("Bayrell.Lang.OpCodes.OpAssign");
	...
	readInc: function(parser)
	{
		...
		if ((look1_content == "++" || look1_content == "--") && parser.parser_base.constructor.isIdentifier(look2_content))
		{
			...
			op_code = new _v0(use("Runtime.Dict").create({"kind":(look1_content == "++") ? _v1.KIND_PRE_INC : _v2.KIND_PRE_DEC,"value":op_code,"caret_start":caret_start,"caret_end":parser.caret}));
		...
		if ((look2_content == "++" || look2_content == "--") && parser.parser_base.constructor.isIdentifier(look1_content))
		{
			...
			var _v1 = use("Bayrell.Lang.OpCodes.OpInc");
			var _v2 = use("Bayrell.Lang.OpCodes.OpInc");
			op_code = new _v0(use("Runtime.Dict").create({"kind":(look2_content == "++") ? _v1.KIND_POST_INC : _v2.KIND_POST_DEC,"value":op_code,"caret_start":caret_start,"caret_end":parser.caret}));
			return use("Runtime.Collection").create([parser,op_code]);
		}
	...
	readCallFunction: function(parser)
	{
		...
		var _v1 = use("Bayrell.Lang.OpCodes.OpPipe");
		if (op_code instanceof _v0 || op_code instanceof _v1)
		{
			return use("Runtime.Collection").create([parser,op_code]);
		}
		return use("Runtime.Collection").create([parser,null]);
	...
	readOperator: function(parser)
	{
		...
		{
			var _v0 = use("Bayrell.Lang.OpCodes.OpContinue");
			return use("Runtime.Collection").create([look,new _v0(use("Runtime.Dict").create({"caret_start":caret_start,"caret_end":look.caret}))]);
		}
	...
	readOperators: function(parser)
	{
		...
		if (token.content == "{")
		{
			...
			op_code = new _v0(use("Runtime.Dict").create({"items":arr.toCollection(),"caret_start":caret_start,"caret_end":parser.caret}));
		}
		else
		{
			var res = this.readOperator(parser);
	...
	readFlags: function(parser)
	{
		...
		var values = new _v0();
		var _v0 = use("Bayrell.Lang.OpCodes.OpFlags");
		var current_flags = _v0.getFlags();
		var res = parser.parser_base.constructor.readToken(parser);
		look = res[0];
		token = res[1];
		...
		var _v0 = use("Bayrell.Lang.OpCodes.OpFlags");
		return use("Runtime.Collection").create([parser,new _v0(values)]);
	},
	/**
	 * Read function args
	 */
	readDeclareFunctionArgs: function(parser,find_ident)
	{
		...
		while (!token.eof && token.content != ")")
		{
			...
			var _v0 = use("Bayrell.Lang.OpCodes.OpDeclareFunctionArg");
			items.push(new _v0(use("Runtime.Dict").create({"pattern":arg_pattern,"name":arg_name,"expression":arg_expression,"caret_start":arg_pattern.caret_start,"caret_end":parser.caret})));
			var res = parser.parser_base.constructor.readToken(parser);
			look = res[0];
			token = res[1];
			if (token.content == ",")
		...
		if (token.content == "use")
		{
			...
			while (!token.eof && token.content != ")")
			{
				...
				if (vars != null && find_ident)
				{
					...
					{
						var _v0 = use("Bayrell.Lang.Exceptions.ParserError");
						throw new _v0("Unknown identifier '" + use("Runtime.rtl").toString(name) + use("Runtime.rtl").toString("'"),ident.caret_start,parser.file_name)
					}
				}
				items.push(name);
				var res = parser.parser_base.constructor.readToken(parser);
	...
	readDeclareFunction: function(parser,has_name)
	{
		...
		/* Restore vars */
		parser = parser.copy({ "vars": save_vars });
		var _v0 = use("Bayrell.Lang.OpCodes.OpDeclareFunction");
		return use("Runtime.Collection").create([parser,new _v0(use("Runtime.Dict").create({"args":args,"vars":vars,"name":name,"result_type":result_type,"expression":expression,"value":op_code,"caret_start":caret_start,"caret_end":parser.caret}))]);
	},
	/**
	...
	tryReadFunction: function(parser,has_name,flags)
	{
		...
		try
		{
			...
			if (flags != null && flags.p_declare || parser.current_class_kind == "interface")
			{
				...
				{
					var _v0 = use("Bayrell.Lang.Exceptions.ParserExpected");
					throw new _v0("Function",caret_start,parser.file_name)
				}
			}
