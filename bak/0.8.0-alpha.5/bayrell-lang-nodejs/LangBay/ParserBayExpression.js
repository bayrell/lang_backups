...
Object.assign(Bayrell.Lang.LangBay.ParserBayExpression,
{
	...
	readBitNot: function(parser)
	{
		...
		{
			var op_code = null;
			var res = parser.parser_base.constructor.readBaseItem(look);
			parser = res[0];
			op_code = res[1];
			var _v0 = use("Bayrell.Lang.OpCodes.OpMath");
			return use("Runtime.Collection").create([parser,new _v0(use("Runtime.Dict").create({"value1":op_code,"math":"!","caret_start":caret_start,"caret_end":parser.caret}))]);
		}
	...
	readBitShift: function(parser)
	{
		...
		while (!token.eof && (token.content == ">>" || token.content == "<<"))
		{
			...
			look_value = res[1];
			var _v0 = use("Bayrell.Lang.OpCodes.OpMath");
			op_code = new _v0(use("Runtime.Dict").create({"value1":op_code,"value2":look_value,"math":math,"caret_start":caret_start,"caret_end":look.caret}));
			parser = look;
			var res = parser.parser_base.constructor.readToken(parser);
			look = res[0];
			token = res[1];
	...
	readCompare: function(parser)
	{
		...
		if (content == "===" || content == "!==" || content == "==" || content == "!=" || content == ">=" || content == "<=" || content == ">" || content == "<")
		{
			...
			look_value = res[1];
			var _v0 = use("Bayrell.Lang.OpCodes.OpMath");
			op_code = new _v0(use("Runtime.Dict").create({"value1":op_code,"value2":look_value,"math":math,"caret_start":caret_start,"caret_end":parser.caret}));
			parser = look;
		}
	...
	readNot: function(parser)
	{
		...
		if (token.content == "not")
		{
			...
			op_code = res[1];
			var _v0 = use("Bayrell.Lang.OpCodes.OpMath");
			return use("Runtime.Collection").create([parser,new _v0(use("Runtime.Dict").create({"value1":op_code,"math":"not","caret_start":caret_start,"caret_end":parser.caret}))]);
		}
	...
	readAnd: function(parser)
	{
		...
		while (!token.eof && (token.content == "and" || token.content == "&&"))
		{
			...
			look_value = res[1];
			var _v0 = use("Bayrell.Lang.OpCodes.OpMath");
			op_code = new _v0(use("Runtime.Dict").create({"value1":op_code,"value2":look_value,"math":"and","caret_start":caret_start,"caret_end":look.caret}));
			parser = look;
			var res = parser.parser_base.constructor.readToken(parser);
			look = res[0];
			token = res[1];
	...
	readOr: function(parser)
	{
		...
		while (!token.eof && (token.content == "or" || token.content == "||"))
		{
			...
			look_value = res[1];
			var _v0 = use("Bayrell.Lang.OpCodes.OpMath");
			op_code = new _v0(use("Runtime.Dict").create({"value1":op_code,"value2":look_value,"math":"or","caret_start":caret_start,"caret_end":look.caret}));
			parser = look;
			var res = parser.parser_base.constructor.readToken(parser);
			look = res[0];
			token = res[1];
	...
	readTernary: function(parser)
	{
		...
		if (token.content == "?")
		{
			...
			var _v0 = use("Bayrell.Lang.OpCodes.OpTernary");
			op_code = new _v0(use("Runtime.Dict").create({"condition":condition,"if_true":if_true,"if_false":if_false,"caret_start":caret_start,"caret_end":parser.caret}));
		}
		return use("Runtime.Collection").create([parser,op_code]);
	},
	/**
