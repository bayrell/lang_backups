...
Object.assign(Bayrell.Lang.LangES6.TranslatorES6Operator,
{
	...
	OpAssign: function(t,op_code,flag_indent)
	{
		...
		if (op_code.kind == __v0.KIND_ASSIGN || op_code.kind == __v1.KIND_DECLARE)
		{
			for (var i = 0;i < op_code.values.count();i++)
			{
				...
				{
					if (t.current_function.isFlag("async"))
					{
						s = item.var_name;
					}
					else
				...
				var __v2 = use("Bayrell.Lang.OpCodes.OpAssign");
				if (!(item.expression == null && op_code.kind == __v2.KIND_DECLARE && t.current_function.isFlag("async")))
				{
	...
	OpReturn: function(t,op_code)
	{
		...
		{
			var content = t.s("var __memorize_value = " + use("Runtime.rtl").toString(s1) + use("Runtime.rtl").toString(";"));
	...
	OpThrow: function(t,op_code)
	{
		var res = t.expression.constructor.Expression(t, op_code.expression);
		t = res[0];
		var content = t.s("throw " + use("Runtime.rtl").toString(res[1]));
		return use("Runtime.Collection").create([t,content]);
	},
	/**
	 * OpTryCatch
	...
	Operator: function(t,op_code)
	{
		...
		var __v8 = use("Bayrell.Lang.OpCodes.OpReturn");
		var __v9 = use("Bayrell.Lang.OpCodes.OpThrow");
		var __v10 = use("Bayrell.Lang.OpCodes.OpTryCatch");
		var __v11 = use("Bayrell.Lang.OpCodes.OpWhile");
		var __v12 = use("Bayrell.Lang.OpCodes.OpInc");
		var __v13 = use("Bayrell.Lang.OpCodes.OpPreprocessorIfCode");
		var __v14 = use("Bayrell.Lang.OpCodes.OpPreprocessorSwitch");
		var __v15 = use("Bayrell.Lang.OpCodes.OpComment");
		if (op_code instanceof __v0)
		{
		...
		else if (op_code instanceof __v1)
		{
			...
			content += use("Runtime.rtl").toString(t.s(op_code.var_name + use("Runtime.rtl").toString(" = ") + use("Runtime.rtl").toString(s1) + use("Runtime.rtl").toString(";")));
			t = t.copy({ "save_op_codes": save_op_codes });
			t = t.copy({ "save_op_code_inc": save_op_code_inc });
			return use("Runtime.Collection").create([t,content]);
		}
		...
		{
			var res = t.expression.constructor.OpCall(t, op_code, false);
			t = res[0];
			if (res[1] != "")
			{
	...
	OpDeclareFunctionBody: function(t,f)
	{
		...
		else if (f.expression)
		{
			...
			{
				s = t.s("var __memorize_value = " + use("Runtime.rtl").toString(expr) + use("Runtime.rtl").toString(";"));
				s += use("Runtime.rtl").toString(t.s(t.expression.constructor.useModuleName(t, "Runtime.rtl") + use("Runtime.rtl").toString("._memorizeSave(\"") + use("Runtime.rtl").toString(t.current_class_full_name) + use("Runtime.rtl").toString(".") + use("Runtime.rtl").toString(f.name) + use("Runtime.rtl").toString("\", arguments, __memorize_value);")));
		...
		{
			var s = "";
			s += use("Runtime.rtl").toString(t.s("var __memorize_value = " + use("Runtime.rtl").toString(t.expression.constructor.useModuleName(t, "Runtime.rtl")) + use("Runtime.rtl").toString("._memorizeValue(\"") + use("Runtime.rtl").toString(t.current_class_full_name) + use("Runtime.rtl").toString(".") + use("Runtime.rtl").toString(f.name) + use("Runtime.rtl").toString("\", arguments);")));
