...
Object.assign(Bayrell.Lang.LangES6.TranslatorES6Expression,
{
	...
	Dynamic: function(ctx, t, op_code, is_call)
	{
		...
		else if (op_code instanceof __v1)
		{
			...
			while (op_code_first instanceof __v4)
			{
			...
			else if (op_code_first instanceof __v8)
			{
				...
				if (op_code_first.kind == __v9.KIND_CLASSREF)
				{
					...
					{
						s_arr.pushValue(ctx, "this" + use("Runtime.rtl").toStr(((!t.is_static_function) ? (".constructor") : (""))));
						prev_kind = "static";
					}
					else if (op_code_first.value == "parent")
					{
						s_arr.pushValue(ctx, this.useModuleName(ctx, t, t.current_class_extends_name));
						prev_kind = "parent";
					}
					else if (op_code_first.value == "self")
					{
						prev_kind = "static";
						s_arr.pushValue(ctx, t.current_class_full_name);
					}
				...
				else if (op_code_first.kind == __v10.KIND_PIPE)
				{
					...
					/* s_arr.pushValue( t.pipe_var_name ~ ".val" ); */
				}
				else
				{
					...
					if (t.modules.has(ctx, op_code_first.value) || op_code_first.kind == __v11.KIND_SYS_TYPE)
					{
						prev_kind = "static";
					}
				}
			...
			if (first_item_complex && t.is_pipe)
			{
				...
				s_arr.pushValue(ctx, first_item);
			}
			...
			for (var i = 0;i < attrs_sz;i++)
			{
				...
				if (attr.kind == __v4.KIND_ATTR)
				{
					...
					if (t.is_pipe && !is_call)
					{
						if (i == attrs_sz - 1)
						{
							s_arr.pushValue(ctx, ".bind(" + use("Runtime.rtl").toStr(first_item) + use("Runtime.rtl").toStr(")"));
						}
						else
						{
							var __v5 = use("Runtime.rs");
							first_item = __v5.join(ctx, "", s_arr);
						}
					}
					else
					{
				...
				else if (attr.kind == __v5.KIND_STATIC)
				{
					if (prev_kind == "var")
					{
						s_arr.pushValue(ctx, ".constructor." + use("Runtime.rtl").toStr(attr.value.value));
					...
					else if (prev_kind == "parent")
					{
						...
						{
							s_arr.pushValue(ctx, "." + use("Runtime.rtl").toStr(attr.value.value) + use("Runtime.rtl").toStr(".bind(this)"));
						}
						else
						{
							s_arr.pushValue(ctx, ".prototype." + use("Runtime.rtl").toStr(attr.value.value) + use("Runtime.rtl").toStr(".bind(this)"));
						}
					}
					else
					{
					...
					if (t.is_pipe && prev_kind != "parent" && !is_call)
					{
						...
						else
						{
							...
							first_item = __v6.join(ctx, "", s_arr);
						}
					}
					prev_kind = "static";
				}
				else if (attr.kind == __v6.KIND_DYNAMIC)
				{
					var res = this.Expression(ctx, t, attr.value);
					t = Runtime.rtl.get(ctx, res, 0);
					var __v7 = use("Runtime.rs");
					var s1 = __v7.join(ctx, "", s_arr);
					var __v8 = use("Runtime.Vector");
					s_arr = new __v8(ctx);
				...
				else if (attr.kind == __v9.KIND_DYNAMIC_ATTRS)
				{
					var __v10 = use("Runtime.Vector");
					var items = new __v10(ctx);
					if (attr.attrs != null)
					{
					...
					var s1 = __v11.join(ctx, "", s_arr);
					var __v12 = use("Runtime.Vector");
					s_arr = new __v12(ctx);
					var __v13 = use("Runtime.rs");
					s_arr.pushValue(ctx, "Runtime.rtl.attr(ctx, " + use("Runtime.rtl").toStr(s1) + use("Runtime.rtl").toStr(", [") + use("Runtime.rtl").toStr(__v13.join(ctx, ", ", items)) + use("Runtime.rtl").toStr("])"));
				}
			}
	...
	OpCall: function(ctx, t, op_code, is_expression)
	{
		...
		if (t.current_function != null && t.current_function.isFlag(ctx, "async") && op_code.is_await && t.isEmulateAsyncAwait(ctx))
		{
	...
	OpPipe: function(ctx, t, op_code, is_expression)
	{
		...
		var res = t.constructor.addSaveOpCode(ctx, t, use("Runtime.Dict").from({"content":t.s(ctx, "var " + use("Runtime.rtl").toStr(var_name) + use("Runtime.rtl").toStr(" = ") + use("Runtime.rtl").toStr(value) + use("Runtime.rtl").toStr(";"))}));
		t = Runtime.rtl.get(ctx, res, 0);
		/* Output items */
		for (var i = 0;i < items.count(ctx);i++)
		{
			...
			else if (op_item.kind == __v3.KIND_METHOD || op_item.kind == __v4.KIND_CALL)
			{
				s2 = "if (!(" + use("Runtime.rtl").toStr(var_name) + use("Runtime.rtl").toStr(" instanceof ") + use("Runtime.rtl").toStr(monad_name) + use("Runtime.rtl").toStr("))");
				...
				value = Runtime.rtl.get(ctx, res, 1);
				var __v5 = use("Bayrell.Lang.OpCodes.OpPipe");
				if (op_item.kind == __v5.KIND_CALL && t.current_function.isFlag(ctx, "async") && op_item.is_async)
				{
				...
				t = t.levelDec(ctx);
				s2 += use("Runtime.rtl").toStr(t.s(ctx, "}"));
				s2 += use("Runtime.rtl").toStr(t.s(ctx, "catch (__err)"));
				s2 += use("Runtime.rtl").toStr(t.s(ctx, "{"));
				t = t.levelInc(ctx);
				s2 += use("Runtime.rtl").toStr(t.s(ctx, var_name + use("Runtime.rtl").toStr("=new ") + use("Runtime.rtl").toStr(monad_name) + use("Runtime.rtl").toStr("(ctx,null,__err);")));
				t = t.levelDec(ctx);
				s2 += use("Runtime.rtl").toStr(t.s(ctx, "}"));
				t = t.levelDec(ctx);
				s2 += use("Runtime.rtl").toStr(t.s(ctx, "}"));
			}
			else if (op_item.kind == __v5.KIND_DEFAULT)
			{
				s2 = "if (" + use("Runtime.rtl").toStr(var_name) + use("Runtime.rtl").toStr(" instanceof ") + use("Runtime.rtl").toStr(monad_name) + use("Runtime.rtl").toStr(" || ") + use("Runtime.rtl").toStr(var_name) + use("Runtime.rtl").toStr(" == null || ") + use("Runtime.rtl").toStr(var_name) + use("Runtime.rtl").toStr(" == undefined)");
				s2 += use("Runtime.rtl").toStr(t.s(ctx, "{"));
				t = t.levelInc(ctx);
				s2 += use("Runtime.rtl").toStr(t.s(ctx, "try"));
				s2 += use("Runtime.rtl").toStr(t.s(ctx, "{"));
		...
		var res = var_name + use("Runtime.rtl").toStr(" instanceof ") + use("Runtime.rtl").toStr(monad_name) + use("Runtime.rtl").toStr(" ? ") + use("Runtime.rtl").toStr(var_name) + use("Runtime.rtl").toStr(".value() : ") + use("Runtime.rtl").toStr(var_name);
