...
class TranslatorPHP extends CommonTranslator{
	...
	OpMethod(op_code){
		if (op_code.value instanceof OpDynamic){
			...
			return "new \\BayrellRtl\\Types\\CallFunc("+rtl.toString(obj)+", "+rtl.toString(this.convertString(name))+")";
		}
		else if (op_code.value instanceof OpStatic){
			var name = op_code.value.name;
			var obj = this.translateRun(op_code.value.value);
			return "new \\BayrellRtl\\Types\\CallFunc("+rtl.toString(obj)+"::class, "+rtl.toString(this.convertString(name))+")";
		}
	...
	OpFunctionArrowDeclare(op_code){
		...
		for (var i = 0; i < op_code.args.count(); i++){
			...
			this.popOneLine();
			use_vars.push("$"+rtl.toString(variable.name));
			ch = ", ";
		}
		res += ")";
		res += "{";
	...
	OpClassInit(class_variables){
		...
		if (this.current_module_name != "BayrellRtl" && this.current_class_name != "CoreObject"){
			if (has_cloneable){
