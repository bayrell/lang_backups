...
class BayrellParserBay extends BayrellCommonParser {
	...
	matchDeclareArgs(){
		...
		while (!this.isLookToken(")") && !this.eof()) {
			...
			rtl.array_push(childs, BayrellCode.op_declare_var(name, type, value, flags));
			if (!this.isLookToken(")")) {
				this.match(",");
			}
		}
	...
	matchLogicExpression(){
		...
		var op = null;
		var right = null;
		if (this.isLookTokenArr([">", "!", "<", "=", "!=", "==", "===", "!==", "<=", ">=", "in", "is", "instanceof"])) {
			op = this.getToken();
			/* 
			TODO: какой то баг
			if (this.isLookToken('=')){
	...
	matchExpression(){
		...
		var is_new = false;
		var is_clone = false;
		var old_is_operation = this._is_operation;
		this._is_operation = true;
		if (this.isLookToken("&")) {
			this.match("&");
			is_link = true;
		}
		...
		this._is_operation = old_is_operation;
		return code_tree;
	}
	/* 
	...
	matchPreprocessorIfCode(){
		...
		var code_str = this._readUntilStringArr(["#endif", "#case", "#endswitch"], false);
		this.getToken();
		if (flag_open_token) {
			this.match("#endif");
		}
