...
Object.assign(Runtime.rtl,
{
	...
	newInstance: function(__ctx, class_name, args)
	{
		...
		args = args.slice(); 
		args.unshift(null);
		if (this.is_ctx) args.unshift(__ctx);
	...
	clone: function(__ctx, val)
	{
		...
		else if (val instanceof Collection)
		{
			...
			{
				if (this.is_ctx) res.push(__ctx, this.clone(__ctx, val[i]));
				else res.push(this.clone(val[i]));
			}
			return res;
		}
		else if (val instanceof Dict)
		{
			...
			{
				if (this.is_ctx) res._map[key] = this.clone(__ctx, val._map[key]);
				else res._map[key] = this.clone(val._map[key]);
			}
			return res;
		}
		...
		{
			if (this.is_ctx) return val.clone(__ctx);
			return val.clone();
		}
		else if (val instanceof Reference)
		{
			if (this.is_ctx) return new Reference(__ctx, val.ref);
		...
		else if (val instanceof CoreObject || typeof val == 'object')
		{
			...
			{
				var key = a[i];
				if (this.is_ctx) res[key] = this.clone(__ctx, val[key]);
				else res[key] = this.clone(val[key]);
			}
			return res;
		}
