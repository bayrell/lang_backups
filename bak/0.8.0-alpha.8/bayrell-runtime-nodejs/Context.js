...
Object.assign(Runtime.Context,
{
	...
	create: function(env, modules, settings)
	{
		...
		if (modules == null)
		{
			...
			{
				modules = (m != null) ? m.keys() : use("Runtime.Collection").from([]);
			}
		}
		/* Get settings */
		if (settings == null)
		...
		/* Base path */
		var __v0 = use("Runtime.rtl");
		var base_path = __v0.attr(settings, use("Runtime.Collection").from(["base_path"]), "", "string");
		if (base_path == "")
		{
			var __v0 = use("Runtime.rtl");
			base_path = __v0.attr(env, use("Runtime.Collection").from(["BASE_PATH"]), "", "string");
		}
		/* Context data */
		...
		if (__v0.is_ctx)
		{
			context = this.newInstance(null, obj);
		}
		else
		{
			context = this.newInstance(obj);
		}
		return context;
	},
	/**
	...
	getModulesEntities: function(modules)
	{
		...
		for (var i = 0;i < modules.count();i++)
		{
			var module_class_name = modules.item(null, i) + use("Runtime.rtl").toString(".ModuleDescription");
			var __v0 = use("Runtime.rtl");
	...
	extendEntities: function(entities)
	{
		...
		{
			var item1 = entities.item(null, i);
			...
			if (item1_class_name == "Runtime.Annotations.Entity")
			{
				...
				{
					var item2 = info.class_info.item(null, j);
					...
					{
						item2 = item2.copy(__ctx, { "name": class_name });
						e.push(null, item2);
					}
				}
			}
	...
	init: function(context)
	{
		...
		/*entities = static::chain(context, "Runtime.Entities", [ context, entities ]);*/
		/* Get providers */
		var providers = use("Runtime.Dict").from({});
		/* Warn static::getProvidersFromEntities(context);*/
		/* Register drivers */
		var drivers = use("Runtime.Dict").from({});
		/* Warn static::getDriversFromEntities(context);*/
		return context.copy(use("Runtime.Dict").from({"entities":entities,"providers":providers,"drivers":drivers,"initialized":true}));
	},
	/**
	 * Start context
	 */
	start: function(context)
	{
		...
		return (__async_t) =>
		{
			...
			{
				return __async_t.ret(__async_t, context.copy(use("Runtime.Dict").from({"started":true})));
			}
			return __async_t.ret_void();
		};
	},
	...
	{
		var arr = context.entities.filter((__ctx, item) => 
		{
		...
		{
			var item = arr.item(i);
			providers.set(null, item.name, item);
		}
	...
	getDriversFromEntities: function(context)
	{
		...
		for (var i = 0;i < arr.count();i++)
		{
			...
			var __v0 = use("Runtime.rtl");
			var driver = __v0.newInstance(class_name, use("Runtime.Collection").from([context]));
			...
			drivers.set(null, item.name, driver);
		}
	...
	createProvider: function(context, provider_name, params)
	{
		...
		if (context.providers.has(provider_name))
		{
			...
			var __v0 = use("Runtime.rtl");
			provider = __v0.newInstance(class_name, use("Runtime.Collection").from([params]));
	...
	getProvider: function(context, provider_name, settings_name)
	{
		...
		var __v0 = use("Runtime.rtl");
		var params = __v0.attr(context.settings, use("Runtime.Collection").from(["providers",provider_name,settings_name]));
	...
	chain: function(context, chain_name, args)
	{
		...
		for (var i = 0;i < entities.count();i++)
		{
			...
			if (item_chain_name != "")
			{
				...
				args = args.setIm(context, args.count() - 1, res);
			}
			else
			{
	...
	chainAwait: function(context, chain_name, args)
	{
		...
		return (__async_t) =>
		{
			if (__async_t.pos() == "0")
			{
				entities = context.entities.filter((__ctx, item) => 
				{
					var __v0 = use("Runtime.Annotations.LambdaChain");
			...
			else if (__async_t.pos() == "1.3.2")
			{
				...
				class_name = arr.get(null, 0, "");
				method_name = arr.get(null, 1, "");
				var __v0 = use("Runtime.rtl");
