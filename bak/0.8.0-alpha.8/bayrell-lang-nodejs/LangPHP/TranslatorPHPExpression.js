...
Object.assign(Bayrell.Lang.LangPHP.TranslatorPHPExpression,
{
	...
	{
		var __v0 = use("Runtime.rs");
		return "\\" + use("Runtime.rtl").toString(__v0.join(__ctx, "\\", this.findModuleNames(__ctx, t, names)));
	},
	/**
	 * OpTypeIdentifier
	 */
	...
	OpIdentifier: function(__ctx, t, op_code)
	{
		...
		else if (op_code.kind == __v1.KIND_VARIABLE)
		{
			...
			return use("Runtime.Collection").from([t,"$" + use("Runtime.rtl").toString(content)]);
		}
	...
	OpDict: function(__ctx, t, op_code, flag_array)
	{
		...
		{
			var __v0 = use("Runtime.rs");
			content = "[" + use("Runtime.rtl").toString(__v0.join(__ctx, ",", values)) + use("Runtime.rtl").toString("]");
		}
		return use("Runtime.Collection").from([t,content]);
	},
	/**
		...
		else if (op_code instanceof __v1)
		{
			...
			for (var i = 0;i < attrs.count(__ctx);i++)
			{
				...
				else if (attr.kind == __v3.KIND_STATIC)
				{
					...
					{
						s += use("Runtime.rtl").toString("->staticMethod(" + use("Runtime.rtl").toString(this.toString(__ctx, attr.value.value)) + use("Runtime.rtl").toString(")"));
					}
					prev_kind = "static";
				}
	...
	OpMath: function(__ctx, t, op_code)
	{
		...
		else
		{
			...
			if (op_math == "~")
			{
				content = op1 + use("Runtime.rtl").toString(" . ") + use("Runtime.rtl").toString(this.rtlToStr(__ctx, t, op2));
			}
	...
	OpTypeConvert: function(__ctx, t, op_code)
	{
		...
		t = res[0];
		var value = res[1];
		content = "\\Runtime\\rtl::to(" + use("Runtime.rtl").toString(value) + use("Runtime.rtl").toString(", ") + use("Runtime.rtl").toString(this.toPattern(__ctx, t, op_code.pattern)) + use("Runtime.rtl").toString(")");
		return use("Runtime.Collection").from([t,content]);
	},
	/**
	 * OpTernary
	...
	OpDeclareFunction: function(__ctx, t, op_code)
	{
		...
		if (op_code.vars != null && op_code.vars.count(__ctx) > 0)
		{
			...
			var __v0 = use("Runtime.rs");
			content += use("Runtime.rtl").toString(" use (" + use("Runtime.rtl").toString(__v0.join(__ctx, ",", vars)) + use("Runtime.rtl").toString(")"));
		}
