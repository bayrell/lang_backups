...
Object.assign(Bayrell.Lang.LangBay.ParserBayBase,
{
	...
	findIdentifier: function(__ctx, parser, name, caret)
	{
		...
		else if (parser.find_ident)
		{
			...
			throw new __v0(__ctx, "Unknown identifier '" + use("Runtime.rtl").toString(name) + use("Runtime.rtl").toString("'"), caret, parser.file_name)
		}
		return kind;
	},
	/**
	...
	readIdentifier: function(__ctx, parser, find_ident)
	{
		...
		{
			var __v0 = use("Bayrell.Lang.Exceptions.ParserExpected");
			throw new __v0(__ctx, "Identifier " + use("Runtime.rtl").toString(token.content) + use("Runtime.rtl").toString(" is reserverd"), token.caret_start.clone(__ctx), parser.file_name)
		}
		name = token.content;
		var kind = "";
		if (find_ident)
		...
		if (f_next(__ctx, token.content))
		{
			...
			if (op_code instanceof __v0)
			{
				...
				if (op_code.kind != __v1.KIND_SYS_TYPE && op_code.kind != __v2.KIND_VARIABLE && op_code.kind != __v3.KIND_MODULE && op_code.kind != __v4.KIND_CLASSREF)
				{
					...
					throw new __v5(__ctx, "Module or variable '" + use("Runtime.rtl").toString(op_code.value) + use("Runtime.rtl").toString("'"), op_code.caret_start.clone(__ctx), parser.file_name)
				}
			}
