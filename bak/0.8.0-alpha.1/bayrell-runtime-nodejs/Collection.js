...
Runtime.Collection = class extends Array
{
	...
	{
		return this;
		if (type == "var" || type == "primitive" || type == "var") return this;
		
		var arr = this.copy();
		for (var i=0; i<arr.length; i++)
		{
			/*if (isBrowser()) arr[i] = Runtime.rtl.convert(arr[i], type, null);
			else arr[i] = rtl.convert(arr[i], type, null);*/
			 arr[i] = use("Runtime.rtl").convert(arr[i], type, null);
		}
		return arr;
	}
	 ...
	 * Each item recursive
	 * @param fn f
	 * @param fn childs Returns childs items
	 * @param fn kind. 1 - Node item first, -1 - Node item last
	 */
