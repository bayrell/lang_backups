...
Object.assign(Bayrell.Lang.LangBay.ParserBayPreprocessor,
{
	...
	readPreprocessorSwitch: function(parser)
	{
		...
		var _v1 = use("Bayrell.Lang.OpCodes.OpPreprocessorIfCode");
		var _v2 = use("Bayrell.Lang.OpCodes.OpPreprocessorSwitch");
		var look = null;
		var token = null;
		var op_code = null;
		var items = new _v0();
		...
		while (token.content == "#case")
		{
			...
			look = res[0];
			token = res[1];
			var ifcode = new _v1(use("Runtime.Dict").create({"condition":condition,"content":content,"caret_start":caret_content,"caret_end":parser.caret}));
			items.push(ifcode);
		}
		/* Restore vars */
		...
		parser = res[0];
		return use("Runtime.Collection").create([parser,new _v2(use("Runtime.Dict").create({"items":items.toCollection(),"caret_start":caret_start,"caret_end":parser.caret}))]);
	},
