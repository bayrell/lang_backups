...
Object.assign(Runtime.RuntimeUtils,
{
	...
	getVariablesNames: function(class_name, flag)
	{
		...
		var names = class_names.reduce((names, item_class_name) => 
		{
			...
			{
				var __v0 = use("Runtime.rtl");
				item_fields = __v0.method(item_class_name, "getFieldsList")(flag);
			}
			catch (_ex)
			{
				if (true)
	...
	getClassIntrospection: function(class_name)
	{
		...
		{
			var __v0 = use("Runtime.rtl");
			info = __v0.method(class_name, "getClassInfo")();
			if (info != null)
			{
		...
		for (var i = 0;i < class_names.count();i++)
		{
			...
			{
				var __v0 = use("Runtime.rtl");
				item_fields = __v0.method(item_class_name, "getFieldsList")(3);
			}
			catch (_ex)
			{
				if (true)
			...
			for (var j = 0;j < item_fields.count();j++)
			{
				...
				var __v0 = use("Runtime.rtl");
				info = __v0.method(item_class_name, "getFieldInfoByName")(field_name);
			...
			{
				var __v0 = use("Runtime.rtl");
				item_methods = __v0.method(item_class_name, "getMethodsList")();
			}
			catch (_ex)
			{
				if (true)
			...
			for (var j = 0;j < item_methods.count();j++)
			{
				...
				var __v0 = use("Runtime.rtl");
				info = __v0.method(item_class_name, "getMethodInfoByName")(method_name);
				appendAnnotations(methods, method_name, info);
			}
		}
		/* To Collection */
		methods = methods.map((item, name) => 
		{
			return item.toCollection();
		});
		fields = fields.map((item, name) => 
		{
			return item.toCollection();
		});
		var __v0 = use("Runtime.Annotations.IntrospectionClass");
		var __v1 = use("Runtime.rtl");
		var __memorize_value = new __v0(use("Runtime.Dict").create({"class_name":class_name,"class_info":class_info.toCollection(),"fields":fields.toDict(),"methods":methods.toDict(),"interfaces":__v1.getInterfaces(class_name)}));
	...
	PrimitiveToObject: function(obj)
	{
		...
		if (obj instanceof __v0)
		{
			...
			var __v1 = use("Runtime.rtl");
			if (!__v1.class_exists(class_name))
			{
				return null;
			}
			var __v1 = use("Runtime.rtl");
			if (!__v1.class_implements(class_name, "Runtime.Interfaces.SerializeInterface"))
			{
				return null;
			}
			/* New instance */
			var __v1 = use("Runtime.rtl");
			var instance = __v1.newInstance(class_name, null);
			/* Assign values */
			var __v1 = use("Runtime.Map");
			var obj = new __v1();
	...
	randomString: function(length, options)
	{
		...
		if (__v0.strpos(options, "a") >= 0)
		{
			s += use("Runtime.rtl").toString("abcdefghjkmnpqrstuvwxyz");
		}
		var __v0 = use("Runtime.rs");
		if (__v0.strpos(options, "b") >= 0)
		{
			s += use("Runtime.rtl").toString("ABCDEFGHJKMNPQRSTUVWXYZ");
		}
		var __v0 = use("Runtime.rs");
		if (__v0.strpos(options, "c") >= 0)
		{
			s += use("Runtime.rtl").toString("1234567890");
		}
		var __v0 = use("Runtime.rs");
		if (__v0.strpos(options, "d") >= 0)
		{
			s += use("Runtime.rtl").toString("!@#$%^&?*_-+=~(){}[]<>|/,.:;\\");
		}
		var __v0 = use("Runtime.rs");
		if (__v0.strpos(options, "e") >= 0)
		{
	...
	getCssHash: function(s)
	{
		...
		for (var i = 0;i < sz;i++)
		{
			...
			var c = __v0.ord(__v1.substr(s, i, 1));
			h = (h << 2) + (h >> 14) + c & 65535;
		}
		var p = 0;
		while (h != 0 || p < 4)
		{
			...
			h = h >> 4;
			var __v0 = use("Runtime.rs");
			r += use("Runtime.rtl").toString(__v0.substr(a, c, 1));
			p = p + 1;
		}
	...
	normalizeUIVector: function(data)
	{
		...
		if (data instanceof __v0)
		{
			...
			for (var i = 0;i < data.count();i++)
			{
				...
				else if (__v3.isString(item))
				{
					var __v4 = use("Runtime.UIStruct");
					var __v5 = use("Runtime.UIStruct");
					var __v6 = use("Runtime.rtl");
					res.push(new __v4(use("Runtime.Dict").create({"kind":__v5.TYPE_RAW,"content":__v6.toString(item)})));
				}
			}
			return res.toCollection();
		}
		else if (data instanceof __v1)
		{
			...
			return new __v2(this.normalizeUI(data));
		}
		else if (__v2.isString(data))
		{
			var __v3 = use("Runtime.Collection");
			return new __v3(this.normalizeUI(data));
		}
		return null;
	},
	/**
	...
	normalizeUI: function(data)
	{
		...
		else if (__v1.isString(data))
		{
			var __v2 = use("Runtime.UIStruct");
			var __v3 = use("Runtime.UIStruct");
			var __v4 = use("Runtime.rtl");
			return new __v2(use("Runtime.Dict").create({"kind":__v3.TYPE_RAW,"content":__v4.toString(data)}));
		}
		return null;
	},
	/* Lambda Functions */
	isInstance: function(class_name)
	{
		...
		{
			var __v0 = use("Runtime.rtl");
			return __v0.is_instance(item, class_name);
		};
	},
	/**
