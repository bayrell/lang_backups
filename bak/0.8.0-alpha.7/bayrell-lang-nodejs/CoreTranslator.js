...
Object.assign(Bayrell.Lang.CoreTranslator.prototype,
{
	...
	s: function(s, content)
	{
		...
		return this.crlf + use("Runtime.rtl").toString(__v0.str_repeat(this.indent, this.indent_level)) + use("Runtime.rtl").toString(s);
	},
	/**
...
Object.assign(Bayrell.Lang.CoreTranslator,
{
	...
	incSaveOpCode: function(t)
	{
		...
		t = t.copy(use("Runtime.Dict").create({"save_op_code_inc":save_op_code_inc}));
	...
	addSaveOpCode: function(t, data)
	{
		...
		var s = new __v0(data);
		t = t.copy(use("Runtime.Dict").create({"save_op_codes":t.save_op_codes.pushIm(s),"save_op_code_inc":save_op_code_inc}));
		return use("Runtime.Collection").create([t,var_name]);
	},
	/**
	 * Clear save op code
	...
	clearSaveOpCode: function(t)
	{
		...
		t = t.copy({ "save_op_codes": new __v0() });
	...
	outputSaveOpCode: function(t, save_op_code_value)
	{
		...
		for (var i = 0;i < t.save_op_codes.count();i++)
		{
			...
			var s = (save.content == "") ? t.s("var " + use("Runtime.rtl").toString(save.var_name) + use("Runtime.rtl").toString(" = ") + use("Runtime.rtl").toString(save.var_content) + use("Runtime.rtl").toString(";")) : save.content;
	...
	saveOpCodeCall: function(t, f, args)
	{
		...
		return use("Runtime.Collection").create([t,save,value]);
	},
