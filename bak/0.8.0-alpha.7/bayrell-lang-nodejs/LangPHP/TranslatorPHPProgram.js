...
Object.assign(Bayrell.Lang.LangPHP.TranslatorPHPProgram,
{
	...
	OpNamespace: function(t, op_code)
	{
		...
		var __v0 = use("Runtime.rs");
		return use("Runtime.Collection").create([t,t.s("namespace " + use("Runtime.rtl").toString(__v0.join("\\", arr)) + use("Runtime.rtl").toString(";"))]);
	},
	/**
	 * OpDeclareClass
	 */
	...
	OpDeclareClassBody: function(t, op_code)
	{
		...
		if (class_kind != __v0.KIND_INTERFACE && op_code.vars != null)
		{
			...
			{
				content += use("Runtime.rtl").toString(t.s("function _init(/*$__ctx*/)"));
				content += use("Runtime.rtl").toString(t.s("{"));
				t = t.levelInc();
				if (t.current_class_extends_name != "")
				{
					content += use("Runtime.rtl").toString(t.s("parent::_init(/*$__ctx*/);"));
				}
				for (var i = 0;i < op_code.vars.count();i++)
				{
					var variable = op_code.vars.item(i);
			...
			if (class_kind == __v1.KIND_STRUCT)
			{
				...
				content += use("Runtime.rtl").toString(t.s("if ($o instanceof \\" + use("Runtime.rtl").toString(__v2.replace("\\.", "\\", t.current_class_full_name)) + use("Runtime.rtl").toString(")")));
				content += use("Runtime.rtl").toString(t.s("{"));
				t = t.levelInc();
				for (var i = 0;i < op_code.vars.count();i++)
				{
