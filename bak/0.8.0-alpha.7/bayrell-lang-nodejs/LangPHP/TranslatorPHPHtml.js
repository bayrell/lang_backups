...
Object.assign(Bayrell.Lang.LangPHP.TranslatorPHPHtml,
{
	...
	OpHtmlComponent: function(t, op_code)
	{
		...
		var model = op_code.attrs.findItem(__v0.equalAttr("key", "@model"));
		if (model)
		{
		...
		{
			var __v0 = use("Runtime.lib");
			var bind = op_code.attrs.findItem(__v0.equalAttr("key", "@bind"));
			if (bind)
			{
		...
		content += use("Runtime.rtl").toString(t.s("/* Component '" + use("Runtime.rtl").toString(tag_name) + use("Runtime.rtl").toString("' */")));
	...
	OpHtmlTag: function(t, op_code)
	{
		...
		{
			var __v0 = use("Runtime.rs");
			attr_s = " " + use("Runtime.rtl").toString(__v0.join(" ", attrs));
		}
		...
		else
		{
			...
			{
				var __v0 = use("Runtime.lib");
				var model_attr = op_code.attrs.findItem(__v0.equalAttr("key", "@model"));
				if (model_attr != null)
				{
