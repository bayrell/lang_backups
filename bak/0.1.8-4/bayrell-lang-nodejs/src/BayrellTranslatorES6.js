...
class BayrellTranslatorES6 extends BayrellTranslator {
	...
	getCallName(name, level){
		return this.getName(name, level);
	}
	...
	op_declare_func(code_tree, level){
		...
		if (name == "constructor") {
			...
			for (var i = 0; i < count(this._constructor_declare_vars); i++) {
				...
				if (isset(code.value)) {
					s = s + this.out(("this." + code.name + " = " + this.run(code.value, (level + 1)) + this._semicolon), (level + 1));
				}
				else {
		...
		while (i < sz) {
			...
			s = s + this.out(("if (!isset(" + obj.name + ")){" + obj.name + " = " + this.run(obj.value, level) + ";}"), (level + 1));
			i = i + 1;
		}
		this._is_func_args = 0;
		this._is_func_args_count = 0;
	...
	op_declare_class(code_tree, level){
		...
		while (i < count(code_tree.childs)) {
			...
			if (code != null && code["op"] == BayrellCode.OP_DECLARE_VAR && code.flags.static) {
				if (isset(code.value)) {
					s = s + this.out((name + "." + code.name + " = " + this.run(code.value, level) + this._semicolon), level);
				}
				else {
