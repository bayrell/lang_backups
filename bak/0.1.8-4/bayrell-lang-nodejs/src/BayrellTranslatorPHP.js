...
class BayrellTranslatorPHP extends BayrellTranslator {
	...
	op_comment(code_tree, level){
		var value = preg_replace("\t", this._ident, code_tree.value);
	...
	getName(name, level, escape){
		...
		if (is_object(name)) {
			s = escape_ch + this.run(name, level);
		}
		else if (strfind(name, "::") > -1) {
			var arr = explode("::", name);
			var sz = count(arr);
			/*  Если все заглавные буквы, то это константа и ее не надо экранировать */
			var field = arr[sz - 1];
			if (count(field) > 0) {
			...
			arr[sz - 1] = escape_ch + arr[(sz - 1)];
			s = implode("::", arr);
		}
		else if (in_array(name, ["null", "false", "true"])) {
			s = name;
		}
		else {
			s = escape_ch + name;
	...
	getCallName(name, level){
		var s = "";
		if (is_object(name)) {
			s = this.run(name, level);
		}
		else if (name == "super") {
	...
	op_assign(code_tree, level){
		var s = this.getName(code_tree.name, level);
		return this.out(s + " = " + this.run(code_tree.value, level) + this._semicolon, level);
	}
	...
	op_declare_var(code_tree, level){
		...
		if (this._is_func_args) {
			var name = this.getName(code_tree.name, level);
			...
			var name = this.getName(code_tree.name, level, !code_tree.flags["const"]);
			if (code_tree.flags["const"]) {
				name = "const " + name;
			}
			else if (code_tree.flags["public"]) {
				name = "public " + name;
			}
			...
			if (code_tree.flags["static"] && !code_tree.flags["const"]) {
				name = "static " + name;
			}
	...
	op_declare_func(code_tree, level){
		...
		if (code_tree.flags["static"]) {
			name = "static function " + func_name;
		}
		else {
	...
	op_call_func(code_tree, level){
		...
		return this.out(this.getCallName(code_tree.name, level) + "(" + args + ")" + this._semicolon, level);
	}
	/*  Операции */
	...
	op_load(code_tree, level){
		return this.getName(code_tree.name, level);
	}
	...
	op_call(code_tree, level){
		...
		return this.getCallName(code_tree.name, level) + "(" + args + ")";
	}
